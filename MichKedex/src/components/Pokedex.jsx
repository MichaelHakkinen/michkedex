...
import './Pokedex.css';
import { IDR, avgP } from './utils';

// ---------------------------------------------------------------------------
// Regional Pokédex definitions
// Each entry: { id (national dex #), name, types }
// ---------------------------------------------------------------------------

// Helper to build a slice of the national dex
const national = (() => {
  // Full list 1-1025 (all gens) - we store the data region-by-region and merge
  return null; // computed below after all regions defined
})();
...
// Build national dex (sorted by id)
const NATIONAL = [...KANTO, ...JOHTO, ...HOENN, ...SINNOH, ...UNOVA, ...KALOS, ...ALOLA, ...GALAR, ...PALDEA];

// Region definitions for the selector
const REGIONS = [
  { key: 'national', label: 'National Pokédex', emoji: '🌐', list: NATIONAL, game: 'All Generations', count: NATIONAL.length },
  { key: 'kanto', label: 'Kanto Pokédex', emoji: '🔴', list: KANTO, game: 'Gen 1 · Red & Blue', count: KANTO.length },
  { key: 'johto', label: 'Johto Pokédex', emoji: '🌟', list: JOHTO, game: 'Gen 2 · Gold & Silver', count: JOHTO.length },
  { key: 'hoenn', label: 'Hoenn Pokédex', emoji: '🌿', list: HOENN, game: 'Gen 3 · Ruby & Sapphire', count: HOENN.length },
  { key: 'sinnoh', label: 'Sinnoh Pokédex', emoji: '💎', list: SINNOH, game: 'Gen 4 · Diamond & Pearl', count: SINNOH.length },
  { key: 'unova', label: 'Unova Pokédex', emoji: '⚫', list: UNOVA, game: 'Gen 5 · Black & White', count: UNOVA.length },
  { key: 'kalos', label: 'Kalos Pokédex', emoji: '✨', list: KALOS, game: 'Gen 6 · X & Y', count: KALOS.length },
  { key: 'alola', label: 'Alola Pokédex', emoji: '🌺', list: ALOLA, game: 'Gen 7 · Sun & Moon', count: ALOLA.length },
  { key: 'galar', label: 'Galar Pokédex', emoji: '⚔️', list: GALAR, game: 'Gen 8 · Sword & Shield', count: GALAR.length },
  { key: 'paldea', label: 'Paldea Pokédex', emoji: '🫐', list: PALDEA, game: 'Gen 9 · Scarlet & Violet', count: PALDEA.length },
];

// ---------------------------------------------------------------------------
const Pokedex = ({ cards }) => {
  const [selectedRegionKey, setSelectedRegionKey] = useState('national');
  const [search, setSearch] = useState('');
  const [selectedType, setSelectedType] = useState('');
  const [selectedPokemon, setSelectedPokemon] = useState(null);

  const currentRegion = useMemo(() => REGIONS.find(r => r.key === selectedRegionKey), [selectedRegionKey]);
  const POKEMON_LIST = currentRegion.list;

  // Group cards by matching pokemon name
  const cardsMap = useMemo(() => {
    const map = {};
    cards.forEach(card => {
      const name = (card.pokemon || '').trim().toLowerCase();
      const matched = NATIONAL.find(p => {
        const pName = p.name.toLowerCase();
        return name === pName || name.includes(pName);
      });
      if (matched) {
        if (!map[matched.id]) map[matched.id] = [];
        map[matched.id].push(card);
      }
    });
    return map;
  }, [cards]);
  ...
  const filteredPokemon = useMemo(() => {
    return POKEMON_LIST.filter(p => {
      ...
      const matchesSearch = p.name.toLowerCase().includes(term) || String(p.id).includes(term);
      const matchesType = selectedType ? p.types.includes(selectedType) : true;
      return matchesSearch && matchesType;
    });
  }, [POKEMON_LIST, search, selectedType]);

  const uniqueTypes = useMemo(() => {
    const types = new Set();
    POKEMON_LIST.forEach(p => p.types.forEach(t => types.add(t)));
    return [...types].sort();
  }, [POKEMON_LIST]);

  const totalCaught = useMemo(() => {
    return POKEMON_LIST.filter(p => cardsMap[p.id]?.length > 0).length;
  }, [POKEMON_LIST, cardsMap]);

  // Reset type filter when region changes (old type might not exist in new region)
    ...
    <div className="pokedex-page">
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px' }}>
        <div>
          ...
          <p style={{ color: 'var(--pv-text-secondary)', fontSize: '13px', marginTop: '4px' }}>
            {currentRegion.game} · {currentRegion.count} Pokémon
          </p>
        </div>
        <div style={{ background: 'var(--pv-bg-primary)', border: '1px solid var(--pv-border-primary)', padding: '10px 16px', borderRadius: 'var(--pv-radius-lg)', textAlign: 'right' }}>
          <div style={{ fontSize: '11px', color: 'var(--pv-text-tertiary)', textTransform: 'uppercase', fontWeight: 600 }}>Dex Progress</div>
          <div style={{ fontSize: '20px', fontWeight: '700', color: 'var(--pv-accent)' }}>
            {totalCaught} <span style={{ fontSize: '13px', color: 'var(--pv-text-secondary)', fontWeight: 'normal' }}>/ {currentRegion.count} Caught</span>
          </div>
        </div>
      </div>
      ...
      <div className="region-selector" style={{ marginBottom: '20px' }}>
        {REGIONS.map(r => (
          <button
            key={r.key}
            className={`region-btn ${selectedRegionKey === r.key ? 'active' : ''}`}
            onClick={() => handleRegionChange(r.key)}
            title={r.game}
          >
            <span className="region-btn-emoji">{r.emoji}</span>
            <span className="region-btn-label">{r.label.replace(' Pokédex', '')}</span>
          </button>
        ))}
      </div>

      {/* Search & Filter Row */}
      <div className="ctrl-row" style={{ marginBottom: '24px' }}>
        <input
          type="text"
      ...
      <div className="dex-grid">
        {filteredPokemon.map(p => {
          const matchingCards = cardsMap[p.id] || [];
          const isCaught = matchingCards.length > 0;
          const imageId = String(p.id).padStart(3, '0');
          const artworkUrl = `https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/other/official-artwork/${p.id}.png`;

          return (
            <div
              key={p.id}
              className={`dex-card ${isCaught ? 'caught' : ''} ${selectedPokemon?.id === p.id ? 'active' : ''}`}
              onClick={() => setSelectedPokemon(selectedPokemon?.id === p.id ? null : { ...p, cards: matchingCards })}
            >
              <div className="dex-number">#{imageId}</div>
              <div className="dex-img-wrap">
                <img src={artworkUrl} alt={p.name} className="dex-img" />
              </div>
              <div className="dex-name">{p.name}</div>
              <div className="dex-types">
                {p.types.map(t => (
                  <span key={t} className={`dex-type-badge ${t.toLowerCase()}`}>{t}</span>
                ))}
              </div>
              {isCaught && (
                <div className="caught-indicator">
                  ✨ {matchingCards.length} {matchingCards.length === 1 ? 'Card' : 'Cards'}
                </div>
              )}
            </div>
          );
      ...
      {/* Selected Pokémon Drawer */}
      {selectedPokemon && (
        <div className="dex-drawer">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px', borderBottom: '1px solid var(--pv-border-primary)', paddingBottom: '12px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              <span style={{ fontSize: '24px' }}>🛡️</span>
              <div>
                <h3 style={{ margin: 0, fontSize: '16px', fontWeight: '700' }}>{selectedPokemon.name} in Your Vault</h3>
                <p style={{ margin: 0, fontSize: '11px', color: 'var(--pv-text-tertiary)' }}>Total owned: {selectedPokemon.cards.length} cards</p>
              </div>
            </div>
            <button className="btn sm" onClick={() => setSelectedPokemon(null)}>Close</button>
          </div>

          {selectedPokemon.cards.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '30px 20px', color: 'var(--pv-text-tertiary)' }}>
              <p style={{ margin: 0 }}>You don't have any copies of {selectedPokemon.name} in your collection yet.</p>
            </div>
          ) : (
            <div className="tbl-container" style={{ maxHeight: '300px', overflowY: 'auto' }}>
              <table className="tbl">
                <thead>
                  <tr>
                    <th>Art</th>
                ...
                <tbody>
                  {selectedPokemon.cards.map(c => (
                    <tr key={c.id}>
                      <td>
                        {c.photos?.[0] ? (
                          <img src={c.photos[0]} alt={c.name} style={{ width: '30px', height: '40px', objectFit: 'cover', borderRadius: '4px' }} />
                        ) : (
                          <span>🃏</span>
                        )}
                      </td>
                      ...
                      <td>{c.set}</td>
                      <td><span className="set-code-tag">{c.setcode}</span></td>
                      <td>{c.cardnum}</td>
                      <td>{c.cond}</td>
                      <td>{c.qty}</td>
                      <td>{IDR(c.buy)}</td>
